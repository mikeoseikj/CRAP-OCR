...
int main(int argc, char *argv[])
{
	...
	printf("\x1B[0;32m");	// Set color print color to green
	printf("\nImage Width: %d pixels\n", deskewed_image->width);
	printf("Image Height: %d pixels\n", deskewed_image->height);
	printf("Skewness: ");
	if(angle == 0)
		printf("Image is not skewed\n\n");
	else if(angle == 1)
		printf("Image is skewed at %d degree clockwise\n\n", angle);
	else
		printf("Image is skewed at %d degrees clockwise\n\n", angle);
